...
- **Stop hook needs a fresh session start.** Claude Code reads `.claude/settings.json` once at session start. If you `/resume` an old session, the hook won't be active — start a new chat instead.
