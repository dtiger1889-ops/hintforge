...
> 🧠 **Run ingestion on a mid-tier, cost-efficient model (Sonnet-class or equivalent) at reasoning effort `high`.** Ingestion makes real judgment calls -- spoiler-tier classification, confidence calibration, routing facts to the right file by vector, reconciling against existing corpus content -- so run it at reasoning effort **`high`** -- the level named `high` (Anthropic `effort: high`, which is Sonnet's default in Claude Code; Codex/GPT-5 `model_reasoning_effort: high`), NOT your runtime's maximum where it exposes higher rungs like `xhigh` / `max` / adaptive. Both levers aim at capable-enough, not overkill: keep the model mid-tier (a flagship over-reasons, drifts off the procedure, and costs more for no better routing) and the effort at `high`, not maxed. Confirm your model and effort setting before triggering.
...
5. **Configured fetch-class anti-bot service (optional, approval-gated).** If the current runtime exposes a service capable of retrieving the Cloudflare-protected page, use it only after the free rungs above fail and only after the user approves the exact URL scope and a per-run paid-call cap. No provider is required or presumed; a service's generic scrape claim is not evidence that it works on Fandom, so verify one returned page before scaling. Record `capture-method: web_fetch`, preserve the canonical Fandom URL as the claim source, and note the service used in the research artifact.
6. **Manual paste from a browser session.** Open the page in a normal browser, select-all, paste into the ingestion file or working chat. For typical ingestion volumes (dozens of pages per game) this is faster than any of the above. Record `capture-method: manual_paste`.
...
|---|---|
| `nav` | `nav/<zone>.md` at **P2+** -- create from `templates/nav_zone.md` if absent. **At P1, per-zone files are NOT created**; P1 `nav` facts route to `nav/architecture.md`'s Chapter-Zone Mapping notes column (see the nav/structure note below the table). |
...
**For `nav` and `structure` facts:** if `nav/` doesn't exist, create it (stub `index.md` + scaffold `architecture.md` from templates). Set `status: research-integrated` on each newly written file. **Phase scoping (resolves the step 4 / step 6 boundary for `nav`):** P1 does NOT create per-zone `nav/<zone>.md` files. P1 `vector: nav` facts arrive at chapter-summary granularity, below the 5-15-gates-per-zone density `templates/nav_zone.md` expects, so they land in `nav/architecture.md`'s Chapter-Zone Mapping "Mission notes" column; creating premature per-zone stubs at P1 would violate the no-placeholder-stub rule in step 5. Per-zone gate-list files are created starting at **P2** (step 6), which folds the P1 notes into full gate lists. After per-zone files exist (P2+), run a consistency pass: every edge declared in a zone file must appear in `architecture.md`'s edge table, and vice versa. Drift between them is a bug.
...
**For claims carrying an `entity:` overlay (at `corpus-core-version: 5` and later):** carry every overlay tag the step-3 classification sub-agent emitted (`entity:`, `entity-status:`, `entity-hidden:`, `achievement:`, `trigger_type:`, and friends) into the destination file **verbatim** -- transcribe the classified output; do not re-derive tags from recall of the source result, do not drop the overlay because the claim also carries a primary `vector:`, and do not bury the overlay inside a `_source:` provenance line (it must sit on the claim). Then, after writing the claim to its primary-vector destination, append a one-line cross-ref to the destination section in **exactly** this form: `> see <class>/<entity-id>.md for full summary`. Emit the mandated string literally -- a freeform equivalent (`-- see \`<class>/<entity-id>.md\``, or the same words in prose) is NOT acceptable, because step 8's sweep greps for the literal `for full summary` token and a semantic paraphrase reads as zero cross-refs. Mechanical and predictable; no judgment call. The cross-ref points readers from the primary-vector home (e.g. a recruitment fact in `nav/<zone>.md`) to the entity's per-entity file. The `enemy` vector and the entity overlay are orthogonal axes: generic-mob combat content continues to route via the `enemy` vector to `mechanics.md` or `warning_tiers.md`, unchanged. Named-NPC combat facts ALSO carry `entity: <id>, entity-status: hostile` and aggregate to `npcs/<id>.md`. The two destinations describe different axes of the same claim, not duplicate writes.
...
- **Split mixed mechanic + lore claims.** When one fact carries both a mechanic (the gate, the choice, the requirement -- `mainline`) and a lore wrapper (the character or story beat behind it -- `lore`), write them as two adjacent claims with their own `category` tags, not one bundled `mainline` claim. Describe gate mechanics terse; push character / event / lore to separate opt-in claims. See [`templates/claim_format.md`](templates/claim_format.md) "Mixed mechanic + lore content." A bundled `mainline` claim leaks the lore half at tier 0 -- and community walkthroughs, which describe these beats openly, push authors toward exactly that bundling, so it needs deliberate resistance.
...
- **P1 ingestion.** First run -- creates `nav/architecture.md` scaffold (zone graph, chapter↔zone mapping, optional content registry, source-language set) plus all per-chapter content distributed by vector. Most content lands here. Per-zone `nav/<zone>.md` files are NOT created at P1 -- P1 `vector: nav` facts go to architecture's Chapter-Zone Mapping notes column; per-zone file creation begins at P2 (see step 4's nav/structure note).
...
**Audit the whole corpus, not only the files you edited this pass -- and emit the ledger.** A prior-phase file carrying `status: research-integrated` is NOT evidence that this phase's supersessions have propagated into it: the status field records that the file was *integrated in its own phase*, not that every later supersession reached it. So for each supersession this phase's result asserts (anything the result marks stale / post-patch / superseded / "earlier guides", or any explicit prior-vs-current contrast), emit a Grep for the affected identifier across **all** claim-bearing files, including ones you did not touch this pass. Before recap, write a **supersession ledger** -- one line per supersession: the identifier, the files grepped, the stale matches found, and the disposition (edited / none found). An empty Grep result is a valid pass; the *absence of the Grep call* is the defect, and the ledger is the falsifiable evidence that the audit actually ran. Do not assert "0 disagreements" or "all propagated" without the ledger behind it.
...
**Step 8 is a field-value check, not a content-propagation check.** A file's `status: research-integrated` value answers "was this file integrated?" -- it does NOT answer "did this phase's supersessions reach this file?" Passing step 8 is not evidence that step 7 ran on a file you did not edit this pass. The two checks ask different questions: step 8 confirms the status-field contract, step 7 confirms content propagation across the whole corpus. Run step 7's whole-corpus supersession audit (above) on its own terms; never treat a clean step-8 status sweep as a proxy for it. Collapsing the two is how a post-patch supersession lands only in the changelog while the stale claim keeps being served at read-time.
...
- `achievements.md` (at `corpus-core-version: 4` and later) -- auto-built from all claims carrying an `achievement:` overlay, organized into the six `trigger_type` H2 sections (`Progression | Branch | Mastery | Collection | Threshold | Discovery`). The completeness target is `research_briefs/achievement_stubs.md` (the Stage 0 stub fetch). For each stub entry: verify the aggregation file has a populated entry with `trigger`, `missable`, `ponr-window`, and `vector-binding` fields. Three end-states are valid: **resolved** (a corpus claim carries the achievement id; aggregation entry promoted to `status: research-integrated`), **deferred** (entry stays `scaffold` with `deferred-to: P3` and a one-line reason -- typical for DLC achievements held over from P1), or **unreachable** (entry recorded in `limitations.md` with what blocked research; aggregation entry stays `scaffold` with `deferred-to: limitations`). Silent scaffold at end of phase is forbidden, same shape as the universal-core rule above. Classify each entry's `trigger_type` per the **Trigger-type classification rules** sub-section below and reorganize the file's flat stub list into the six trigger-type sections (omitting sections with no entries). If `research_briefs/achievement_stubs.md` does not exist (Stage 0 was skipped, or this is a v1-v3 corpus migrating to v4), write the honest empty-statement: "No achievement stub list captured at Stage 0; achievement coverage deferred to maintainer-initiated refetch." Hidden achievements (`achievement-hidden: yes`) keep their name in the aggregation file but the read-time renderer gates the heading entirely below `progression`-tier; the integrator writes them in full regardless of current tier per the "write content, gate display" rule in step 5. **Precondition-free completeness self-check (mandatory -- runs whether or not the stub file exists).** After the six trigger-type sections are written, extract every achievement id/number populated in the aggregation file and assert the set is gap-free and duplicate-free: for an N-achievement game the ids must equal `{1..N}` with no missing id and no id written twice. A duplicated row keeps the total row count at N while concealing a dropped achievement, so a count-only sanity check is not sufficient -- compare the *set of ids*, not the row count. This check needs no external target and must run even when `research_briefs/achievement_stubs.md` is absent. **Fresh platform recount (mandatory at end of phase, before any coverage claim).** The stub file and the id-set check above are both products of this same run, so a wrong stub count validates wrong entries and certifies them as resolved. Before writing any coverage line, re-fetch the platform's live achievement list (the store's global-achievements page, or the platform's equivalent) and compare NAME SETS against the aggregation file: every corpus entry must match a live name, every live name must have a corpus entry, and the live total must equal the corpus total. An extra corpus name, a missing live name, or a count mismatch is a hard stop -- remove entries the platform does not have, add the missing ones as `deferred` stubs, then re-run the comparison -- never a "resolved" line. Record the fetch date and live count in the file header (`stub_source`, `stub_fetched`, `stub_count`). If the live list cannot be fetched this pass, the header says so and the coverage line reads `unverified against live platform list`. Rationale: a corpus was once certified "all resolved / 0 deferred" from its own stub file while a quarter of its entries did not exist on the platform -- base-game names had been copied into expansion sections with invented triggers, and nothing in the pass ever looked outside the run's own files. **Third recovery branch (before the honest empty-statement):** `setup_wizard.md` Stage 0 has historically written the achievement stub list into `achievements.md` itself rather than into `research_briefs/achievement_stubs.md`, and the aggregation rewrite above overwrites that list. So when the stub file is absent, first diff the just-written file against the in-context pre-overwrite copy of `achievements.md` (it was read into context earlier this pass) and recover the stub list from there; only fall back to the empty-statement if no stub list is recoverable from either location. **Do NOT assert "All N achievements resolved" (or any coverage count) in the file header or the CHECKPOINT changelog unless this set check actually ran and passed.**
...
- **Entity overlays.** **Existence precondition -- run this BEFORE the agreement check.** If any entity-class folder (`npcs/`, `factions/`, `crew/`, `reputation/`) holds a populated per-entity file, the corpus must contain (a) at least one matching `entity: <id>` overlay claim AND (b) at least one literal `> see <class>/<entity-id>.md for full summary` cross-ref. A whole-corpus grep for `entity:` -- or for `for full summary` -- that returns zero while populated per-entity files exist is a **FAIL**, not a clean pass: it means step 4's overlays/cross-refs were never emitted, the per-entity files were composed straight from the research result, and the entity-aggregation contract is inert. A matching `entity:` overlay is necessary but not sufficient -- the literal `for full summary` cross-ref must also be present, since a freeform paraphrase satisfies neither the reader's navigation nor this check. Only once the precondition passes: for every `entity: <id>` claim, Grep the corpus for that id. Every match should agree with the aggregated per-entity file: recruitment-window claim in `<class>/<id>.md` matches the gate trigger in zone files; status-history entries align with zone-file gate descriptions; named-individual rule applied consistently across files referencing the entity.
- **Achievement overlays** (at `corpus-core-version: 4` and later). For every `achievement: <id>` claim, Grep the corpus for that id. The `trigger`, `missable`, `ponr-window`, and `vector-binding` fields in `achievements.md` must match the per-claim metadata in the primary-vector destination file. **Warning (not a failure) -- zero `achievement:` overlays corpus-wide.** `achievement:` is optional (`templates/claim_format.md`), so its absence is not itself a violation -- but if `achievements.md` is populated while a corpus-wide grep for `achievement:` returns zero, the aggregation had nothing to build from and was necessarily composed by re-reading the research result rather than aggregating from tagged claims. Flag it in the recap (the achievements file is then NOT machine-checkable against the corpus) and lean on the precondition-free set-completeness self-check in the `achievements.md` aggregation bullet above, which does not depend on overlays.
...
**Record the sweep, don't narrate it.** The consistency sweep counts only if its evidence is captured. For each grep run, record the actual command and its numeric result count in the step-12 recap and the CHECKPOINT changelog (e.g. `grep -rc 'entity:' . -> 14 across 8 files`; `grep -rc 'for full summary' . -> 14`; `grep -rc 'achievement:' . -> 0`). A prose assertion that the sweep was "spot-checked" or "looks consistent enough" without the command + count is not an acceptable substitute -- a narrated-but-unrun sweep is exactly the failure this gate exists to prevent, and recording the counts makes a skipped sweep visibly absent instead of narratable as done.
